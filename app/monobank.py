...
from settings import MONOBANK_PATH, MONOBANK_TOKEN, ACCOUNT, PERIOD
...
def get_all_statements():
    ...
    for account in client_info["accounts"]:
        ...
        # TODO: Implement custom names for accounts. Like White, Black etc.
        # account_name = account["custom_name"]
        account_name = account_id
        try:
            statement = json.loads(
                get_account_statement(account_id, from_timestamp, to_timestamp)
            )
        except Exception:
            continue
        ...
        with open(f"{MONOBANK_PATH}/client_info.json", "w") as file:
            client_info["latest_synchronization"] = to_timestamp
            json.dump(client_info, file, ensure_ascii=False)
